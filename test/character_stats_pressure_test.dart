...
void main() {
  group('Character Stats Validation - Attributes', () {
    test('Base Attribute Rolls - 1000 Characters (Valid Range 3-10)', () async {
      ...
      attributeRanges.forEach((attribute, values) {
        ...
        print('  $attribute: min=$min, max=$max, avg=${avg.toStringAsFixed(2)}');

        // Base roll is 3 + 1d8 = 3-10 (before bonuses)
        // With bonuses from schools/deployments, max can be higher
        expect(min, greaterThanOrEqualTo(3), reason: '$attribute minimum too low');
        expect(max, lessThanOrEqualTo(20), reason: '$attribute maximum suspicious');
      });
    ...
    test('School Bonuses - Strength +1 Per School', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        final characterId = uuid.v4();
        final baseChar = _createBaseCharacter(characterId, 'Germany', 'Test $i');
        final character = await QuickBuildService.generateQuickCharacter(
          characterId,
          'Rifleman',
          baseChar,
        ...
        if (baseAttributes != null && baseAttributes is Map) {
          ...
          if (finalStrength < baseStrength + expectedBonus) {
            schoolBonusErrors++;
            print('  ⚠️ Character $i: Expected Strength +$expectedBonus from $schoolCount schools');
            print('     Base: $baseStrength, Final: $finalStrength, Difference: ${finalStrength - baseStrength}');
          }
        }
      }

      print('School bonus errors: $schoolBonusErrors / $testCount');
      expect(schoolBonusErrors, 0, reason: 'School Strength bonuses not applying correctly');
    ...
    test('Elite School Bonuses - Knowledge +1 For Ranger Equivalent', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        if (hasEliteSchool) {
          ...
          if (baseAttributes != null && baseAttributes is Map) {
            ...
            final finalKnowledge = character.attributes['Combat Knowledge'] ?? 0;

            // SOF gets: Ranger (+1) = at least +1 Knowledge
            if (finalKnowledge >= baseKnowledge + 1) {
              correctBonuses++;
            } else {
              print('  ⚠️ SOF Character $i: Expected Knowledge +1 minimum from elite school');
      ...
      expect(eliteSchoolCount, greaterThan(0), reason: 'No elite schools found in SOF characters');
      expect(correctBonuses, eliteSchoolCount, reason: 'Elite school Knowledge bonuses not applying correctly');
      print('✅ Elite school Knowledge bonuses verified\n');
    });

    test('Officer Promotion Bonuses - Strength +1, Agility +1, Knowledge +1', () async {
      const testCount = 200;
      final uuid = const Uuid();
      var officerCount = 0;
      var correctBonuses = 0;
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        if (rank.contains('Lieutenant') || rank.contains('Captain') || rank.contains('O-')) {
          officerCount++;
          ...
          if (baseAttributes != null && baseAttributes is Map) {
            ...
            if (strengthBonus >= 1 && agilityBonus >= 1 && knowledgeBonus >= 1) {
              correctBonuses++;
            } else {
              print('  ⚠️ Officer $i: Expected +1 to Str/Agi/Know minimum');
              print('     Bonuses: Str +$strengthBonus, Agi +$agilityBonus, Know +$knowledgeBonus');
            }
          }
        }
      }
      ...
      if (officerCount > 0) {
        expect(correctBonuses, equals(officerCount), reason: 'Officer promotion bonuses not consistent');
      }
    ...
    test('Deployment Awards - Knowledge Bonuses (+1, +2, +3)', () async {
        ...
        if (expectedAwardBonus > 0) {
          ...
          if (baseAttributes != null && baseAttributes is Map) {
            ...
            if (actualBonus >= expectedAwardBonus) {
              ...
              print('  ⚠️ Character $i: Expected +$expectedAwardBonus Knowledge from awards');
              print('     Base: $baseKnowledge, Final: $finalKnowledge, Actual bonus: $actualBonus');
            }
          }
        }
      }
      ...
      expect(correctBonuses, equals(awardBonusCount), reason: 'Award Knowledge bonuses not applying correctly');
  ...
  group('Character Stats Validation - Skills (Prowess, Tactics, Instincts)', () {
    test('Prowess Skills - Small Arms Progression', () async {
      ...
      for (final specialty in specialties) {
        ...
        final baseChar = _createBaseCharacter(characterId, 'US', 'Test $specialty');
        final character = await QuickBuildService.generateQuickCharacter(
          characterId,
          specialty,
          baseChar,
        ...
        final specialtyDetail = character.enlistment['specialty']?.toString() ?? '';

        // Expected minimums:
        // Rifleman: 3, Sniper: 4
        // SOF/Agent: Varies by initial specialty (Rifleman/Sniper: 4-5, Medical/Radio: 1-2)
        if (specialty == 'Rifleman') {
          expect(smallArms, greaterThanOrEqualTo(3), reason: 'Rifleman should start with Small Arms 3');
          print('  $specialty: Small Arms = $smallArms (expected ≥3) ✓');
        } else if (specialty == 'Sniper') {
          expect(smallArms, greaterThanOrEqualTo(4), reason: 'Sniper should start with Small Arms 4');
          ...
          expect(smallArms, greaterThanOrEqualTo(1), reason: '$specialty should have Small Arms skill');
          if (specialtyDetail.contains('Rifleman') || specialtyDetail.contains('Sniper')) {
            expect(smallArms, greaterThanOrEqualTo(4), reason: '$specialty with Rifleman/Sniper should have Small Arms ≥4');
            print('  $specialty ($specialtyDetail): Small Arms = $smallArms (expected ≥4 for combat specialty) ✓');
          } else {
            // Medical or Radio Operator - Small Arms is secondary skill
            print('  $specialty ($specialtyDetail): Small Arms = $smallArms (support specialty, Small Arms not primary) ✓');
          }
        }
      }
    ...
    test('Tactics Skills - Explosives Progression (EOD, Breacher Schools)', () async {
      print('\n=== EXPLOSIVES SKILL PROGRESSION ===');

      // Test EOD specialty
      final eodId = const Uuid().v4();
      final eodChar = _createBaseCharacter(eodId, 'US', 'EOD Test');
      final eod = await QuickBuildService.generateQuickCharacter(eodId, 'EOD', eodChar);
      final eodExplosives = eod.skills['Explosives'] ?? 0;

      // EOD gets +3 Explosives
      expect(eodExplosives, greaterThanOrEqualTo(3), reason: 'EOD should have Explosives +3');
      ...
      for (var deployment in deployments) {
        if (deployment is Map && deployment['school'] != null) {
          final school = deployment['school'] as String;
          if (school.contains('Breacher') || school.contains('Explosives +2')) {
            hasBreacher = true;
            break;
          }
        }
      ...
      if (hasBreacher) {
        expect(eodExplosives, greaterThanOrEqualTo(5), reason: 'EOD with Breacher school should have Explosives ≥5');
        print('  EOD with Breacher: Explosives = $eodExplosives (expected ≥5) ✓');
      }
    ...
    test('Tactics Skills - Spying Progression (Agent)', () async {
      ...
      final agent = await QuickBuildService.generateQuickCharacter(agentId, 'Agent', agentChar);
      final spying = agent.skills['Spying'] ?? 0;

      // Agent gets +3 Spying
      expect(spying, greaterThanOrEqualTo(3), reason: 'Agent should have Spying +3');
    ...
    test('Instincts Skills - Fires Progression (JTAC)', () async {
      ...
      final jtac = await QuickBuildService.generateQuickCharacter(jtacId, 'JTAC', jtacChar);
      final fires = jtac.skills['Fires'] ?? 0;
      final radioOps = jtac.skills['Radio Ops'] ?? 0;

      // JTAC gets +3 Fires, +1 Radio Ops
      expect(fires, greaterThanOrEqualTo(3), reason: 'JTAC should have Fires +3');
      expect(radioOps, greaterThanOrEqualTo(1), reason: 'JTAC should have Radio Ops +1');
      print('  JTAC: Fires = $fires (expected ≥3), Radio Ops = $radioOps (expected ≥1) ✓');
    ...
    test('Combat Experience - Per Deployment Progression', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        final deploymentCount = (character.enlistment['deployments'] as List?)?.length ?? 0;
        final combat = character.skills['Combat'] ?? 0;

        // Each deployment should add +1 Combat
        if (combat != deploymentCount) {
          incorrectCombat++;
          print('  ⚠️ Character $i: $deploymentCount deployments, Combat = $combat (expected $deploymentCount)');
        }
      }

      print('Incorrect Combat values: $incorrectCombat / $testCount');
      expect(incorrectCombat, 0, reason: 'Combat experience not matching deployment count');
    ...
    test('Training Skill - Promotion Bonuses', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        final hasSergeant = rank.contains('Sergeant') || rank.contains('E-5') || rank.contains('E-6');
        final hasOfficer = rank.contains('Lieutenant') || rank.contains('Captain') || rank.contains('O-');
        ...
        if (hasSergeant || hasOfficer) {
          ...
          if (training >= 1) {
            ...
            print('  ⚠️ Character $i ($rank): Training = $training (expected ≥1)');
          }
        }
      }
      ...
      if (promotionCount > 0) {
        expect(correctTraining, equals(promotionCount), reason: 'Training bonuses not applying to promotions');
      }
    ...
    test('SOF Highest Skill Bonus - +1 To Best Skill', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        if (highestValue >= 4) {
          ...
          print('  ⚠️ SOF $i: Highest skill = $highestValue (expected ≥4 for boosted skill)');
        }
      }

      print('SOF characters with high skill values: $correctBonuses / $testCount');
      expect(correctBonuses, greaterThan(testCount * 0.8), reason: 'SOF highest skill bonus may not be applying');
    ...
    test('Ability Calculations - Knowledge→Tactics, Wisdom→Instincts', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        final abilities = character.enlistment['abilities'] as Map<String, dynamic>? ?? {};
        ...
        if (tacticsAbility != expectedTactics) {
          ...
          if (i < 5) {  // Only print first 5 errors to avoid spam
            ...
            print('     Expected: $expectedTactics (Knowledge:$knowledge + Combat:$combat + Training:$training + Rifleman:$riflemanBonus)');
        ...
        if (instinctsAbility != expectedInstincts) {
          ...
          if (i < 5) {  // Only print first 5 errors to avoid spam
            ...
            print('     Expected: $expectedInstincts (Wisdom:$wisdom + Training:$training + Combat:$combat)');
      ...
      expect(tacticsErrors, 0, reason: 'Tactics ability not correctly calculated from Knowledge');
      expect(instinctsErrors, 0, reason: 'Instincts ability not correctly calculated from Wisdom');
  ...
  group('Character Stats Validation - Edge Cases & Stacking', () {
    test('No Stat Stacking - Multiple Characters Same Seed', () async {
      ...
      final avgStrength = (character1.attributes['Strength']! + character2.attributes['Strength']!) / 2;
      final avgSmallArms = (character1.skills['Small Arms']! + character2.skills['Small Arms']!) / 2;

      print('  Character 1: Strength=${character1.attributes['Strength']}, Small Arms=${character1.skills['Small Arms']}');
      print('  Character 2: Strength=${character2.attributes['Strength']}, Small Arms=${character2.skills['Small Arms']}');
      print('  Averages: Strength=${avgStrength.toStringAsFixed(1)}, Small Arms=${avgSmallArms.toStringAsFixed(1)}');

      // Values should be within reasonable range (not doubling)
      expect(character1.attributes['Strength'], lessThan(20), reason: 'Strength stacking detected');
      expect(character2.attributes['Strength'], lessThan(20), reason: 'Strength stacking detected');
      expect(character1.skills['Small Arms'], lessThan(15), reason: 'Small Arms stacking detected');
      expect(character2.skills['Small Arms'], lessThan(15), reason: 'Small Arms stacking detected');
    ...
    test('Base vs Final Stats - Deployment Bonuses Only', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        if (baseAttributes != null && baseSkills != null) {
          ...
          if (baseStrength < 3 || baseStrength > 15) {
            baseCheckErrors++;
            print('  ⚠️ Character $i: Base Strength out of range: $baseStrength');
          }
          ...
          if (finalStrength < baseStrength) {
            baseCheckErrors++;
            print('  ⚠️ Character $i: Final Strength ($finalStrength) < Base Strength ($baseStrength)');
          }
          ...
          (baseSkills as Map).forEach((key, value) {
            ...
            if (finalValue < value) {
              baseCheckErrors++;
              print('  ⚠️ Character $i: Final $key ($finalValue) < Base $key ($value)');
            }
          });
        }
      }
      ...
      expect(baseCheckErrors, 0, reason: 'Base vs Final stat validation failed');
      print('✅ Base vs Final stats validated\n');
    });

    test('All Specialties - Stat Consistency Check', () async {
      final specialties = ['Rifleman', 'Heavy Weapons', 'Sniper', 'Medical', 'EOD', 'JTAC', 'SOF', 'Agent'];
      print('\n=== ALL SPECIALTIES STAT CONSISTENCY ===');

      for (final specialty in specialties) {
        final characterId = const Uuid().v4();
        final baseChar = _createBaseCharacter(characterId, 'US', '$specialty Test');
        final character = await QuickBuildService.generateQuickCharacter(
          characterId,
          specialty,
          baseChar,
        ...
        expect(character.attributes['Strength'], greaterThan(0), reason: '$specialty missing Strength');
        expect(character.attributes['Agility'], greaterThan(0), reason: '$specialty missing Agility');
        expect(character.attributes['Combat Wisdom'], greaterThan(0), reason: '$specialty missing Combat Wisdom');
        expect(character.attributes['Combat Knowledge'], greaterThan(0), reason: '$specialty missing Combat Knowledge');

        // Verify core skills exist
        expect(character.skills.containsKey('Small Arms'), true, reason: '$specialty missing Small Arms skill');
        expect(character.skills.containsKey('Combat'), true, reason: '$specialty missing Combat skill');

        // Verify specialty-specific skills
        if (specialty == 'EOD') {
          expect(character.skills['Explosives'], greaterThanOrEqualTo(3), reason: 'EOD missing Explosives bonus');
        } else if (specialty == 'JTAC') {
          expect(character.skills['Fires'], greaterThanOrEqualTo(3), reason: 'JTAC missing Fires bonus');
        } else if (specialty == 'Agent') {
          expect(character.skills['Spying'], greaterThanOrEqualTo(3), reason: 'Agent missing Spying bonus');
        } else if (specialty == 'SOF') {
          expect(character.skills['Training'], greaterThanOrEqualTo(1), reason: 'SOF missing Training bonus');
        }
    ...
    test('Extreme Case - Multiple High Bonuses (SOF Agent)', () async {
      ...
      expect(strength, inInclusiveRange(6, 20), reason: 'Agent Strength out of expected range');
      expect(knowledge, inInclusiveRange(4, 18), reason: 'Agent Knowledge out of expected range');
      expect(spying, greaterThanOrEqualTo(3), reason: 'Agent should have Spying ≥3');
      expect(training, greaterThanOrEqualTo(2), reason: 'Agent should have Training ≥2');
      expect(combat, inInclusiveRange(4, 7), reason: 'Agent should have 4-6 deployments (Combat)');
    ...
    test('Rapid Generation - 500 Characters With Stat Verification', () async {
      ...
      for (int i = 0; i < testCount; i++) {
        ...
        // Quick sanity checks
        if (character.attributes['Strength']! < 3 || character.attributes['Strength']! > 20) {
          statErrors++;
        }
        if (character.skills['Small Arms']! < 3 || character.skills['Small Arms']! > 15) {
          statErrors++;
        }
        if (character.skills['Combat']! < 0 || character.skills['Combat']! > 10) {
          statErrors++;
        }
      }
      ...
      print('Generated $testCount characters in ${stopwatch.elapsedMilliseconds}ms');
